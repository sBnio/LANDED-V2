...
export function CostEstimator() {
  ...
  const optionals = useMemo(() => [
    { 
      ...
      range: `~${formatAmt(800)}–${formatAmt(1200)}`, 
      amount: 1000, 
      tooltip: "Many UAE universities include health insurance. Check with your Student Affairs office first." 
    },
    { id: "airport", name: "Airport Transfer (one way)", range: `~${formatAmt(80)}–${formatAmt(150)}`, amount: 115 },
    { id: "groceries", name: "Initial Groceries & Household Setup", range: `~${formatAmt(300)}–${formatAmt(500)}`, amount: 400 },
    { id: "textbooks", name: "Textbooks & Stationery (first semester)", range: `~${formatAmt(200)}–${formatAmt(500)}`, amount: 350 },
    { 
      id: "emergency", 
...
Still need to pay: ${formatAmt(stillNeedToPay)} ${currencyCode}
Already sorted: ${formatAmt(totalSorted)} ${currencyCode}  
Optional costs: ${formatAmt(totalOptionalsInclude)} ${currencyCode}
Estimated total: ${formatAmt(estimatedTotal)} ${currencyCode}
      ...
      >
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="text-right hidden sm:block">
            <p className="font-bold text-slate-900">{formatAmt(stillNeedToPay)} {currencyCode}</p>
      ...
      {isExpanded && (
        <div className="p-4 space-y-8 animate-in slide-in-from-top-2 fade-in duration-200 overflow-y-auto">
                        ...
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="text-right shrink-0">
                            <span className={cn(
                              ...
                              {formatAmt(item.amount)} {currencyCode}
                            </span>
                            {isSorted && (
              ...
              {optionals.map((item) => {
                ...
                return (
                  <div key={item.id} className={cn(
                    ...
                    isInc ? "border-amber-200 bg-amber-50/30" : "border-slate-100 bg-slate-50 border-dashed"
                  )}>
                    <div className="flex items-start gap-2 pt-0.5">
                      <span className={cn(
                        "w-1.5 h-1.5 rounded-full mt-1.5 shrink-0",
                        isInc ? "bg-amber-500" : "bg-slate-300"
                      )} />
                      <div>
                         <div className="flex items-center gap-1.5">
                         ...
                         <label className="flex items-center gap-2 mt-2 cursor-pointer group/label w-max">
                            <div className="w-4 h-4 rounded border flex items-center justify-center shrink-0 border-slate-300 group-hover/label:border-amber-400 transition-colors bg-white">
                              <input 
                                type="checkbox" 
                                className="opacity-0 absolute w-0 h-0"
                                checked={!!isInc}
                              ...
                              />
                              {isInc && <CheckCircle2 className="w-3.5 h-3.5 text-amber-600" />}
                            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
             <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex flex-col justify-center relative overflow-hidden group">
                <div className="absolute inset-0 bg-gradient-to-br from-amber-400/0 to-amber-400/10" />
                <span className="text-[10px] font-black uppercase tracking-widest text-amber-600/80 mb-1">What You Still Need to Pay</span>
                <span className="text-2xl font-black text-amber-700">{stillNeedToPay.toLocaleString()} <span className="text-sm font-bold text-amber-600/60">{currencyCode}</span></span>
             </div>
             ...
             <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 flex flex-col justify-center">
                ...
                <span className="text-xl font-bold text-emerald-700">{totalSorted.toLocaleString()} <span className="text-xs font-semibold text-emerald-600/60">{currencyCode}</span></span>
             </div>
             ...
             <div className="bg-slate-100 border border-slate-200 rounded-xl p-4 flex flex-col justify-center">
                ...
                <span className="text-xl font-bold text-slate-700">{estimatedTotal.toLocaleString()} <span className="text-xs font-semibold text-slate-500/60">{currencyCode}</span></span>
             </div>
          </div>
